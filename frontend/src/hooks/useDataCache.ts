...
export function useDataCache<T = any>(options: CacheOptions = {}): UseCacheReturn<T> {
  ...
  const getFromStorage = useCallback((fullKey: string): CacheEntry<T> | null => {
  ...
  // Set to localStorage
  const setToStorage = useCallback((fullKey: string, entry: CacheEntry<T>) => {
    if (!persist) return;
  ...
  const removeFromStorage = useCallback((fullKey: string) => {
    if (!persist) return;
  ...
  // Preload data into cache
  const preload = useCallback(async (key: string, fetcher: () => Promise<T>): Promise<T> => {
    // Check cache first
