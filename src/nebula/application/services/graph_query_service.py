...
from nebula.db import GraphSnapshot
...
class GraphQueryService:
    ...
    async def ensure_active_snapshot(self, db: AsyncSession) -> GraphSnapshot:
    ...
    async def rebuild_active_snapshot(self, db: AsyncSession) -> GraphData:
        user = await _get_default_user(db)
        if user is None:
            raise ValueError("No user found")
        version, graph_data, timeline_data = await self.builder.build_payload(db)
